...
export async function buildAssetPlanFromMakerContract(qualityIntent = {}, assetContract = null) {
    ...
    for (const slot of asArray(assetContract?.slots)) {
        ...
        pushRequest(requests, {
            ...
            size: isBackground ? undefined : (slot.size || null),
            width: isBackground ? Number(slot.width || BACKGROUND_DEFAULT_WIDTH) : (slot.width || undefined),
            height: isBackground ? Number(slot.height || BACKGROUND_DEFAULT_HEIGHT) : (slot.height || undefined),
