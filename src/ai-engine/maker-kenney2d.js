...
export function kenney2dSpritePromptBlock(resolution = {}) {
    ...
    if (resolution.background) lines.push(`  - "background" — ${resolution.background.keys.length} decorative parallax pieces (clouds/hills/scenery — draw with drawParallax(), see WORLD; NEVER tile them edge-to-edge)`);
    lines.push("  HOW: import { sprite, animatedSprite, tile, count, tileGround, scatterProps, drawParallax } from './sprite.ts'. sprite(ctx, 'player', x, y) draws the base pose centered at (x,y); animatedSprite(ctx, 'player', 'walk', t, x, y) cycles its frames (t = elapsed seconds); tile(ctx, 'tiles', i, x, y) draws one specific tile. Options: { size, scale, flipX, anchor:'bottom' } — use anchor:'bottom' so feet rest on the ground, and pass `size` to fit sprites to the world (native art is small — e.g. `size: 40` for a character). Each returns false if unavailable, so you may draw a fallback. NEVER draw a colored rectangle for anything listed above — a real sprite always looks better. Code-draw ONLY what has no sprite (HUD numbers via sdf2d, simple particles). Image smoothing is auto-set by the kernel per art style — do not touch it.");
    lines.push("  WORLD (fill it so the game looks FINISHED, not an empty void — but match the camera): TOP-DOWN games → every frame before entities call `tileGround(ctx, canvas.width, canvas.height)` to tile the whole floor with real ground tiles, then `scatterProps(ctx, propList)` (~6-12 fixed prop positions chosen ONCE at init) to dress it. SIDE-SCROLLER / PLATFORMER games → do NOT tile a full-screen floor: draw a code gradient SKY (this is correct, not a failure), then `drawParallax(ctx, { viewW: canvas.width, baselineY, count: 3 })` for a sparse far layer of scenery, then build platforms/ground from tile/platform sprites, then entities. tileGround uses ONLY the seamless `tiles` role and returns false if there are none — then fall back to a solid/gradient fill. NEVER tile the decorative `background` pieces across the screen (they are not seamless — it looks broken).");
    return lines.join('\n');
}
