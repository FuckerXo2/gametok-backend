...
export function buildMakerAssetContract(templateContract = null, qualityIntent = {}) {
    ...
    if (templateId === 'canvas-runner') {
        return {
            ...
            sourceOfTruth: 'template asset slots consumed by DreamAssets at runtime',
            hardRules: [
                'HUD, score, distance, lives, jump, and slide controls are code-rendered only.',
                'Runner physics, obstacle collisions, collectible pickups, speed, and score are code-defined.',
                'Generated images decorate runner, obstacles, collectibles, and background only.',
            ],
            slots: [
                {
                    id: 'runner_player',
                    ...
                    size: 128,
                    transparent: true,
                    description: `Side-view runner character. Isolated single subject in running pose, readable silhouette, transparent background, no text, no UI, no scenery. ${artStyleText(qualityIntent)}`.trim(),
                    consumedBy: 'runner player renderer via DreamAssets role player',
                    fallback: 'code-rendered runner body',
                },
                {
                    id: 'runner_obstacle',
                    required: false,
                    ...
                    size: 96,
                    transparent: true,
                    description: `Runner obstacle or hazard. Isolated single object, transparent background, no text, no UI, no scenery. ${artStyleText(qualityIntent)}`.trim(),
                    consumedBy: 'obstacle renderer via DreamAssets role enemy',
                    fallback: 'code-rendered obstacle',
                },
                {
                    id: 'runner_collectible',
                    required: false,
                    assetType: 'sprite',
                    role: 'item',
                    category: 'item',
                    size: 64,
                    transparent: true,
                    description: `Runner collectible or reward item. Isolated centered object, transparent background, no text, no UI. ${artStyleText(qualityIntent)}`.trim(),
                    consumedBy: 'collectible renderer via DreamAssets role item',
                    fallback: 'code-rendered collectible',
                },
                {
                    id: 'runner_background',
                    required: false,
                    assetType: 'background',
                    role: 'background',
                    category: 'environment',
                    ...
                    height: 1344,
                    transparent: false,
                    description: `Portrait mobile runner background with clear track area in lower third. No text, no HUD, no controls, no characters. ${artStyleText(qualityIntent)}`.trim(),
                    consumedBy: 'background layer via DreamAssets role background',
                    fallback: 'code-rendered runner background',
                },
            ],
        };
    }
    if (templateId === 'canvas-arcade-shooter') {
        return {
            ...
            sourceOfTruth: 'template asset slots consumed by DreamAssets at runtime',
            hardRules: [
                'HUD, score, wave, health, joystick, and fire button are code-rendered only.',
                'Player movement, projectiles, enemies, collisions, waves, and score are code-defined.',
                'Generated images decorate player, enemies, effects, projectiles, and background only.',
            ],
            slots: [
                {
                    id: 'shooter_player',
                    ...
                    size: 128,
                    transparent: true,
                    description: `Top-down or front-facing arcade shooter player ship/hero. Isolated single subject, readable mobile silhouette, transparent background, no text, no UI, no scenery. ${artStyleText(qualityIntent)}`.trim(),
                    consumedBy: 'player renderer via DreamAssets role player',
                    fallback: 'code-rendered player avatar',
                },
                {
                    ...
                    size: 128,
                    transparent: true,
                    description: `Arcade shooter enemy or threat. Isolated single subject, transparent background, no text, no UI, no scenery. ${artStyleText(qualityIntent)}`.trim(),
                    consumedBy: 'enemy renderer via DreamAssets role enemy',
                    fallback: 'code-rendered enemy avatar',
                },
                {
                    id: 'shooter_effect',
                    required: false,
                    ...
                    size: 96,
                    transparent: true,
                    description: `Projectile impact or muzzle effect. Isolated effect, transparent background, no text, no UI. ${artStyleText(qualityIntent)}`.trim(),
                    consumedBy: 'projectile/impact renderer via DreamAssets role effect',
                    fallback: 'code-rendered projectile and burst',
                },
                {
                    id: 'shooter_background',
                    required: false,
                    assetType: 'background',
                    role: 'background',
                    category: 'environment',
                    ...
                    height: 1344,
                    transparent: false,
                    description: `Portrait mobile arcade shooter background with readable playfield. No text, no HUD, no controls, no characters. ${artStyleText(qualityIntent)}`.trim(),
                    consumedBy: 'background layer via DreamAssets role background',
                    fallback: 'code-rendered shooter background',
                },
            ],
        };
    }
