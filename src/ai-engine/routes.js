            ...
            if (runEvidence?.success) {
                ...
                if (hollowReason && turnNumber < maxTurns && hollowForceRetries < 1) {
                    ...
                    const directRepairTask = bareSeed
                        ...
                        : `You have written NO game code — every tool call was a read, zero files edited. What ships right now is the bare placeholder stub, NOT the game in the prompt. This is a 2D CANVAS game: BUILD IT NOW by implementing the full loop in src/main.ts — the state object, input handlers, spawning/entities, collisions, scoring, win/lose — and DRAW it all on the canvas every frame (getAssetImage(...) + ctx.drawImage for sprites, drawBackground() for the background, DOM/CSS for HUD). There are no src/game or src/entities modules in this scaffold — it is single-file. Stop reading and start writing src/main.ts.`)
                        : unwiredModules
                    ...
                    if (isImplementTurn && turnNumber < implementTurns) {
                        const polishTask = {
                            ...
                            directRepairTask: isThreeJsLane
                                ...
                                : isPhaser2D
                                ? `Your game loop works and compiles! Now use write_file to upgrade visuals so it looks DESIGNED, not programmer-art. You MUST edit files this turn — do NOT just read. This is a 2D CANVAS game using REAL SPRITE ASSETS. Specifically: (1) Ensure all game entities are drawn using the provided getAssetImage() sprites. You MAY use src/sdf2d.ts (e.g. bakeSDF, makeHudPanel) for crisp UI panels and buttons. sdf2d.ts is a UI TOOLKIT — NEVER use it for game entities. (2) Polish the HUD, buttons and panels in src/styles.css (DOM/CSS) to match the game's palette — rounded panels, readable type, subtle gradients/shadows. KEEP CSS VALID — balanced braces, complete @keyframes. (3) Strengthen MOOD with tasteful code-drawn VFX (sparks, thrust particles, subtle screen shake) around the real sprites. (4) Ensure the background is rendering beautifully using drawBackground(). Do NOT break the existing game loop — only ENHANCE visuals. Start writing immediately.`
                                : `Your game loop works and compiles! Now use write_file to upgrade visuals so it looks DESIGNED, not programmer-art. You MUST edit files this turn — do NOT just read. This is a 2D CANVAS game (NO Three.js, NO WebGL, NO bloom, NO src/systems/Hud.ts — do not reference any of those). Specifically: (1) Replace bare ctx.fillRect() UI panels and buttons with crisp baked-SDF shapes — import { bakeSDF, makeHudPanel } from './sdf2d.ts' and bake them once at init for clean anti-aliased edges, soft glow and outlines. sdf2d.ts is a UI TOOLKIT — use it for HUD elements, NOT game entities. (2) Polish the HUD, buttons and panels in src/styles.css (DOM/CSS) to match the game's palette — rounded panels, readable type, subtle gradients/shadows; do NOT bolt on generic sci-fi neon unless the concept is sci-fi. KEEP CSS VALID — balanced braces, complete @keyframes — a broken stylesheet fails the build. (3) Strengthen MOOD with the code-drawn background, a gentle vignette and color cohesion, plus tasteful feedback (sparkles on success, a small shake on failure, floating score). Do NOT break the existing game loop — only ENHANCE visuals. Start writing immediately.`,
                        };
                        runEvidence.success = false;
