...
export function buildMakerAgentImplementPrompt({
    ...
    return [
        ...
        '- You own the full mobile layout: index.html structure, src/styles.css, and all gameplay logic.',
        '- BUILD A ROBUST MULTI-FILE ARCHITECTURE: Use the write_file tool to create separate files for scenes (src/scenes/BootScene.ts, GameScene.ts) and entities (src/entities/Player.ts). DO NOT cram everything into main.ts. Keep individual files under 300 lines to prevent output truncation. The system enforces a hard 350-line limit per file — writes exceeding this are rejected.',
        '- START WRITING CODE IMMEDIATELY: The scaffold main.ts and styles.css are included below — do NOT waste rounds calling read_file on files you already have. Your first tool call should be write_file to create entity/scene files. Every read_file round costs time and tokens.',
