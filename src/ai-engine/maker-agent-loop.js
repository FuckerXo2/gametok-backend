...
export function buildThreeDRulesBlock(foundation = null) {
    ...
    return [
        ...
        "- BUILD MODELS AS VOXELS (the no-asset way to look GOOD): a smooth low-poly primitive (a sphere, a capsule, a stretched box) reads as cheap and unfinished; a BLOCKY VOXEL model reads as premium (think Crossy Road / Minecraft). For every notable entity — player ship/character, enemies, asteroids, pickups, props — prefer building it from colored grid cells with the kernel helper voxelModel(cells, { size }) where cells is [{ x, y, z, color }]: it returns ONE InstancedMesh (one draw call) you parent to your entity Group and move/rotate. Sculpt a recognizable silhouette from ~12-60 cells (ship = long body + wings + cockpit; asteroid = a rough lump of grey/brown cells; gem = a small faceted cluster). Vary the per-cell colors for shading. Add a few SEPARATE emissive MeshStandardMaterial boxes for glowing bits (engines, eyes, windows) so the bloom lights them. Use buildVoxelField(scene, cells) for large terrain/fields. Smooth primitives are a fallback only when a blocky form genuinely doesn't fit.",
