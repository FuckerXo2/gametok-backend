...
export function buildThreeDRulesBlock(foundation = null) {
    ...
    return [
        ...
        "- DO NOT read_file src/threeAssets.ts or src/bootstrap.ts — read-only kernel files. Full API: createThreeStage(canvas) → { renderer, scene, camera, composer, bloom, resize, render }, ALREADY called at the top of main.ts. It sets a sky background, fog, hemisphere + directional light + SOFT SHADOWS, a PBR environment map (reflections), and a BLOOM post-processing pipeline. RENDER WITH stage.render() (the composer — applies bloom + tone mapping); NEVER call renderer.render(scene, camera) directly or you lose the glow. threeAssets.ts also EXPORTS voxel builders you SHOULD use — import { voxelModel, buildVoxelField, disposeObject3D } from './threeAssets.ts' (voxelModel(cells,{size}) → one InstancedMesh for an entity model; buildVoxelField(scene,cells) → a terrain/field; disposeObject3D(obj) → free GPU memory before scene.remove on EVERY recycled obstacle/spent projectile so the game never leaks). The seed FollowCamera (src/systems/Camera.ts) has impulse(amount) — call it on hits/boosts for a clamped camera kick.",
        ...
        "- PREMIUM PROCEDURAL MATERIALS (no image textures): color surfaces with MeshStandardMaterial({ color, metalness, roughness }) so they catch real light + the kernel's environment reflections (metal cars/ships ~metalness 0.7 roughness 0.3; matte ground ~roughness 0.95). Make anything that should GLOW — neon, signs, lights, pickups, engines, crystals — EMISSIVE: MeshStandardMaterial({ color, emissive: '#rrggbb', emissiveIntensity: 1.2 to 2.2 }); the kernel's bloom turns emissive surfaces into real glow. EMISSIVE IS FOR ACCENTS ONLY (engine glow, thruster, cockpit lights, neon trim, the pickup) — NEVER make a whole body/ship/character emissive and never push emissiveIntensity so high the object blooms into an unreadable WHITE blob; the main surfaces stay lit PBR (metalness/roughness) so the shape reads as a solid object. NEVER call getDreamTexture/makeBillboard/applySkybox, never set material.map, never read DREAM_IMAGES for visuals.",
        '- Distinguish player, obstacles, coins, lanes, and pickups by COLOR + SHAPE + size. A clean small palette reads as premium — never leave geometry untinted default-white.',
        "- SMOOTH PREMIUM MODELS (this is the target look — clean lit low-poly, like a premium mobile game): build each notable entity (player ship/character, enemies, props, pickups) as a model COMPOSED of several smooth primitives fused into a RECOGNIZABLE SILHOUETTE — never one stretched box or lone sphere. e.g. a ship = a tapered hull (cone/cylinder) + swept wings + a cockpit canopy + engine nacelles; a character = body + head + limbs. Use enough segments that curves read smooth (sphere/cylinder ~16-24 segments), and skin everything in MeshStandardMaterial (metalness/roughness) so the kernel's light + reflections + soft shadows do the work. A single primitive reads as cheap; a composed, well-proportioned, lit model reads as premium. (Voxel is an OPTION only for an intentionally BLOCKY/retro game — voxelModel(cells,{size}) / buildVoxelField(scene,cells) — but SMOOTH is the default; do not voxelize a sci-fi/realistic game.)",
        "- MODEL DETAIL IN THREE TIERS (every hero/notable asset): PRIMARY form = the readable silhouette; SECONDARY structure = panels, trim, ridges, fins, tubes, sockets, cockpit, engines that fit the role; TERTIARY = small emissive accents/decals visible at the gameplay camera distance (not noise). Give materials purposeful CONTRAST — vary roughness/metalness/color/emissive across parts so nothing is one flat uniform shade. A primitive-dominant scene (bare boxes/cones/spheres/flat planes) is an automatic quality fail.",
        "- HERO RECIPE (player ship/vehicle): tapered hull (extruded/lathe/scaled, not a box) + a distinct nose (wedge/intake/blade) + a cockpit/core (glass dome or faceted canopy) + engines (cylinders with nozzle rings and an inner EMISSIVE disc + trail) + wings/fins (beveled plates) + small decals (panel lines, a faction mark). HERO RECIPE (character): torso + pelvis + head/helmet + tapered limbs grouped under named shoulder/elbow/knee joints + an identity cue (visor/crest/color-block) + material zones (skin/fabric/armor/metal/emissive). REJECT your own model if the hero is just a box with two cylinders and a glow, or stacked spheres with no costume/joints.",
        "- GAME FEEL (tune it, don't just configure it): camera keeps the player + the next threat/reward + the route all visible; add brief camera IMPULSES on hits/near-misses/boosts that ease back fast (clamp shake — never nauseating); give every core action layered feedback (a hit = flash + knockback + sound; a pickup = pop + glow + count-up). Ramp difficulty/speed over time. Material contrast first (matte vs glossy, metal vs plastic, bright trim vs dark body) before leaning on post effects.",
        "- WORLD = DEPTH + A PROP KIT, not one flat lane: compose FOREGROUND, MIDGROUND, BACKGROUND layers so the scene has depth (parallax/scenery near and far). Build a small REUSABLE prop kit (a few authored modules you vary + repeat) — never one stretched box or one repeated skyline primitive. Use the kernel's lights/fog/shadows to clarify depth and mood without hiding gameplay.",
        "- COMPLETENESS GATE (a tech demo is a fail): the FIRST frame is the playable game — never a title/landing screen; the player can act within ~5 seconds. The core loop MUST have an objective, score/progress, a fail + retry path, difficulty pacing, and FEEDBACK on every core action and state change (hit, pickup, near-miss, fail, milestone — via motion + color + the HUD, not one channel). Ship at least THREE visually distinct obstacle/enemy types each with its own telegraph, and at least TWO distinct reward/pickup types with collection feedback — not one recolored shape.",
        "- MOBILE (phone webview is the target): keep touch controls reachable by thumbs and clear of the safe-area edges and the player's focal/threat area; canvas uses touch-action:none and Pointer Events; never let page scroll/zoom steal gameplay input; HUD and controls respect env(safe-area-inset-*). Dynamic HUD values must not shift layout.",
        '- PERF BUDGET (phone webview): MeshStandardMaterial is the default (the kernel already provides bloom + env reflections + soft shadows — do NOT add your own EffectComposer/post-processing or a second renderer). Opt meshes into shadows with castShadow/receiveShadow. Total triangles under ~200k, reuse geometries/materials, never allocate new objects inside the render loop.',
        '- WORLD AXES / FORWARD DIRECTION (critical — get the sign right or the game plays in reverse): the kernel camera sits BEHIND the player (+Z) looking toward -Z, so FORWARD = -Z (into the screen, away from the camera); back/toward-camera = +Z; +X = right; +Y = up. For ANY forward-motion game (runner, racer, endless driver, flyer) the player/vehicle and the world must ADVANCE along -Z: move forward with `pos.z -= speed * dt` — NEVER `pos.z += speed * dt`, which drives toward the camera and makes the car look like it is REVERSING. Spawn obstacles/traffic/scenery AHEAD at large negative z and slide them toward +Z so they rush past the camera; recycle each once its z passes behind the camera (z > camera.z). The chase camera follows at the player z + a positive offset (behind).',
        '- MOVEMENT PLANE & FEEL (match the genre — do NOT blindly keep the placeholder ground movement): the seed Input.ts/Player.ts move the player on the GROUND plane (X = sideways, Y-drag mapped to Z depth, no vertical). That is correct ONLY for ground games (runner, driver, top-down). For a FLYING / SPACE / SHOOTER / shmup game the player must roam the SCREEN plane facing the camera: horizontal input = X (left/right), VERTICAL input = Y (UP/DOWN) — NEVER map vertical drag to Z depth (that makes up/down feel dead, the player looks stuck on a thin line). Remap the input axis to fit. FEEL: movement must be snappy and the playfield GENEROUS — the player should be able to reach most of the visible frame (roughly the full width and height in view), not a tiny clamped box. Tune speed + bounds to the camera so the player clearly traverses the screen; a player pinned near center that barely moves is a failure.',
        ...
        "- HUD / UI (DOM in #hud, never Three.js text) — build a GAME INTERFACE, not a dev dashboard or generic stat-card boxes. The seed src/systems/Hud.ts ALREADY renders a themed corner score + a Retry game-over overlay in #hud — EXTEND it (wire it to your game state, add what the game needs); never leave it bare and never delete the visible score. Order by gameplay priority: status (health/lives) > objective (score/progress/timer/wave) > feedback > flavor. Composition zones: top-LEFT = objective/timer/progress, top-RIGHT = score/combo/currency (+ pause), bottom corners = touch buttons if any, center-top = brief event banners (combo/warnings). Use FIXED-WIDTH numeric containers (font-variant-numeric: tabular-nums) for score/timer/health so values NEVER shift layout. Theme to the game's world/palette with consistent color roles (danger/reward/boost); keep all UI OUT of the play path and off the player/threats; respect safe-area insets. Required states: a live gameplay HUD AND a game-over (and win, if relevant) overlay with a clear primary Retry/Continue action. The first frame is the playable game, not a title/landing screen.",
        ...((lane.includes('runner') || lane.includes('surfer') || lane.includes('dash')) && isFreeBuildMode() ? [
            '- RUNNER (FREE BUILD — you own the whole game, build it like a real project): organize the code across MULTIPLE FILES under src/ — e.g. src/game/Game.ts, src/systems/ (input, camera, collision, spawn, hud), src/entities/ (player, obstacle, pickup), src/world/ (terrain, scenery). Create whatever modules make it clean; write_file each. main.ts is the THIN ENTRY that wires everything: it imports your modules, calls createThreeStage, owns the loop + probe, and re-exports stepGame/renderAll/resetGame. Aim for a genuinely good-looking, fun snowboard/runner — the result is shipped. QUALITY BAR: player with a STRONG READABLE SILHOUETTE from the chase camera (body + board + limbs + head, not one box); 2-3 visually DISTINCT obstacle types (tree/rock/rail) each with its own silhouette + telegraph; a collectible/pickup with a clear shape + collect feedback; layered foreground/midground/background scenery for speed + depth; smooth chase camera with follow lag. HARD CONTRACT (or the build is rejected): use createThreeStage(canvas) from "./threeAssets.ts" (returns { renderer, scene, camera, composer, bloom, resize, render } — lights, soft shadows, PBR environment + bloom already provided); use MeshStandardMaterial (metalness/roughness) and give glowing things (neon/pickups/lights/engines) an EMISSIVE color (emissiveIntensity 1.2-2.2) so the kernel bloom lights them up; no image textures, no getDreamTexture; an animation loop (requestAnimationFrame) that calls stage.render() every frame (the composer applies bloom — do NOT call renderer.render directly); main.ts must `export function stepGame(dt)`, `export function renderAll()`, `export function resetGame()`; and set window.__GAMETOK_TEMPLATE_PROBE__ = { snapshot(){ return { score, gameOver, started, renderCalls: renderer.info.render.calls, ... } }, step(dt){ stepGame(dt); renderAll(); return this.snapshot(); }, reset(){ resetGame(); return this.snapshot(); } }. import "./styles.css" in main.ts. Touch + keyboard input. Keep state in ONE place (a single game-state object passed to systems) so it is never undefined. After all files are written and tsc is clean, call finish_inspection.',
        ] : (lane.includes('runner') || lane.includes('surfer') || lane.includes('dash')) ? [
            '- RUNNER (SINGLE FILE): src/main.ts already runs as a working snowboard/runner game. You OWN exactly one region: the five gameplay functions between the `// ===== GAMETOK:EDIT START =====` and `// ===== GAMETOK:EDIT END =====` markers — setupScene, createObstacle, movePlayer, checkCollisions, updateCamera. Spend ALL your effort there and make them genuinely good (this is graded on the result): a themed player with a STRONG READABLE SILHOUETTE from the chase camera (not a single box — give it body/board/limbs/head subassemblies); 2-3 visually DISTINCT obstacle types via createObstacle (e.g. tree, rock, rail) each with its own silhouette; a collectible/coin variant with a clear shape; and layered foreground/midground/background scenery so speed and depth read. Keep each function\'s exact name and parameters; setupScene MUST set refs.player and add a ground; createObstacle MUST return a mesh added to the scene; gameplay reads the obstacle array as state.obstacles. The pre-wired engine OUTSIDE those markers (state, input, game loop, render, reset, probe) is assembled for you automatically — do NOT rewrite it, do NOT redeclare state, do NOT add a second probe; just keep the GAMETOK:EDIT and GAMETOK:SACRED marker comments in place so the assembler can find your region. Flat colors only (MeshLambertMaterial hex), no imports beyond THREE, no getDreamTexture.',
        ] : []),
