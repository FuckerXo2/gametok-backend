...
export function buildThreeDRulesBlock(foundation = null) {
    ...
    return [
        ...
        '== ENVIRONMENT MANDATE (READ FIRST — the #1 reason these games look unfinished) ==',
        "- YOU ARE BUILDING BLIND: no screenshot is ever shown to you and NOTHING will tell you the scene looks empty — the sandbox happily accepts a flat, white/void-backed scene that merely boots. That is the most common failure mode. So DEFEND AGAINST IT ON PURPOSE: before gameplay you MUST build a FULL environment that FILLS the visible volume out to a fog/backdrop boundary. A bare ground plane, a few props floating in white/grey, or an untextured box room is an AUTOMATIC FAILURE. When unsure, OVER-build the world — density and a real backdrop are never the thing that breaks a 3D game; emptiness is.",
        "- CLASSIFY YOUR SCENE from the prompt + setting and follow the ONE matching recipe (all use code, NO image assets):",
        "   [OUTDOOR / OPEN] city, streets, ocean, space, desert, fields, sky, stadium/arena: (1) a FAR BACKDROP — createShaderBackground/createSDFBackground with a fragment shader written for THIS exact setting (skyline, ocean horizon, nebula, dunes), or createSky for natural daylight/sunset; (2) GROUND — createTerrain for natural land (place props with .heightAt) or a proceduralTexture'd plane for urban/arena floors; (3) DENSITY to the horizon via InstancedMesh + scatter() (buildings, trees, rocks, crowd, debris, stars) — thousands in one draw call; (4) fog COLOUR-MATCHED to the backdrop so the world fades softly and never hard-edges into void.",
        "   [CITY specifically — GTA / racing / superhero/flight over a city] THERE IS NO CITY HELPER, SO BUILD IT (this is well within your ability — pure procedural geometry makes a great city): place DOZENS of buildings as InstancedMesh boxes with VARIED height/width/depth across a grid that fills the view out to the fog line; give them EMISSIVE WINDOW GRIDS (a second emissive InstancedMesh, or an emissive proceduralTexture('checker'/'dots')) so they read as LIT skyscrapers, not grey slabs; lay a ground plane with road markings (proceduralTexture stripes/checker), add instanced streetlights (post + small emissive box), and fog matched to the sky. A handful of boxes on white is the failure; a dense, lit skyline is the bar.",
        "   [INTERIOR / ROOM] room, lab, shop, cabin, dungeon: NO shader-sky backdrop — you are INDOORS. Texture EVERY surface (floor + walls + ceiling) with proceduralTexture (wood/plaster/tile/concrete); set a MOOD with recoloured/dimmed stage.sunLight + stage.hemisphereLight, a lit lamp (emissive box + a PointLight) and gentle fog; then FURNISH IT FULLY per the interior-furnishing rule below. An untextured white box with one floating prop is the classic failure.",
        "   [STYLIZED / BLOCKY world] Minecraft / voxel / low-poly worlds: build the whole terrain AND world with buildVoxelField (one InstancedMesh, thousands of coloured cells) — a blocky ground with hills, water, trees and structures, NOT a flat plane. Voxel's home is the WORLD, not single props.",
        "- THE TEST (apply it to your own scene): if the camera flew around, would you see a believable, FILLED place — or objects suspended in emptiness? It MUST be the former before you call the build done.",
        ...
        '- INTERIOR / ROOM SCENES — FURNISH THEM (a room with one desk and empty walls reads BROKEN, not minimalist): a real room has a bed, desk, chair, bookshelf, lamp, rug, plants, wall art, a window and small clutter. Use loadModel() for any Kenney furniture listed above (bed/desk/chair/bookcase/lamp/rug/mirror, tint to your palette), and composed Box/Cylinder groups or voxelModel() for anything not offered. Add a floor + wall material (proceduralTexture), a lit lamp, and trim. NEVER ship a near-empty box with a single floating prop — that is the #1 thing that makes these games look unfinished.',
        ...
        "- SDF SHADER BACKGROUND (code, NO assets, infinite variety — you are GOOD at this, use it): import { createShaderBackground, createSDFBackground } from './threeAssets.ts'. Write a fullscreen fragment shader for the EXACT backdrop the prompt implies — a neon city skyline, an ocean horizon, a desert canyon, a warp tunnel, a starfield, raymarched hills — and pass it: createShaderBackground(stage, { fragmentShader, uniforms }). It owns all the plumbing (fullscreen quad locked behind the gameplay, u_time in seconds + u_res in pixels, self-animating on its own rAF). createSDFBackground(stage, {...}) is a complete worked raymarched-hills example: READ it to learn the pattern (raymarch / fbm / sky gradient / sun / fog), then write your OWN shader for THIS game's setting — do not ship hills for a city. This is the strongest, cheapest cinematic backdrop there is; your playable road/ground/entities stay your own geometry IN FRONT of it.",
        ...
        "- NO WHITE-OUT (critical): a washed-out, over-bright scene is WORSE than a plain one. Keep emissive for SMALL accents only (lights, engines, pickups, edges) — NEVER set whole bodies, the road, the ground, or large surfaces emissive. Do NOT use bright/white fog (it washes the frame); fog color must match the sky and stay muted. Do NOT stack createSky + max emissive + dense glow together. Most surfaces should be lit normally by sunLight/hemisphereLight, not self-glowing. If you're unsure, err DARKER and more saturated — never brighter.",
...
export function buildMakerAgentImplementPrompt({
    ...
    return [
        ...
        ...(!is3D ? [
            `- KERNEL MODULES ALREADY IN THIS PROJECT (do NOT search for them, do NOT doubt they exist, do NOT skip them because you are unsure): src/sdf2d.ts (crisp anti-aliased panels/buttons/sprites with glow) and src/iso.ts (isometric grid). They are on disk right now — import them directly. ${phaser2dBlock ? "WARNING: This game uses REAL ASSETS for entities. You may use sdf2d for UI/HUD panels ONLY. NEVER use it for game entities." : "NEVER fall back to flat ctx.fillRect()/ctx.arc() for HUD panels, buttons, pickups or any element the player looks at: that flat look is exactly the cheap, washed-out 'programmer-art' result we are avoiding. Use src/sdf2d.ts."}`,
            `- 2D SDF POLISH (code, NO assets, crisp + glowing) — src/sdf2d.ts IS present, import it: import { bakeSDF, sdCircle, sdRoundRect, sdCapsule, opSmoothUnion, opRotate, makeGem, makeOrb, makeHudPanel } from './sdf2d.ts'. ${phaser2dBlock ? "CRITICAL: Use bakeSDF ONLY for UI panels and buttons. DO NOT use it for game entities (players/enemies/items), which must use getAssetImage()." : "For any code-drawn sprite, pickup, HUD panel or shape that should look premium, BAKE it once at init from a signed-distance field —"} bakeSDF(w, h, sdf, { fillTop, fillBottom, outline, outlineWidth, glow, glowSize, highlight }) returns a canvas you ctx.drawImage(sprite, x, y) like a normal sprite, with perfect anti-aliasing + soft glow + clean outline for free. Compose forms with opSmoothUnion (metaballs) / opRotate before baking. makeGem/makeOrb/makeHudPanel are worked examples — read them, then bake the shapes THIS game needs. Prefer this over bare ctx.arc()/fillRect for anything the player looks at; it is the cheapest way to make a code-drawn 2D game read as designed, not programmer-art.`,
        ...
        ...(!is3D ? [
            "- STATE KEY COLLISIONS (a classic FATAL bug — it has killed whole jobs): the foundation's required state fields (score, coins, lives, health, ammo, wave…) are SCALARS the HUD reads — keep them scalar. NEVER reuse one of those names for an entity array. Using `coins` as BOTH the score number AND the array of coin sprites produces duplicate-property / 'number is not assignable to []' tsc errors that eat every repair turn. Name entity arrays distinctly: coinEntities / coinSprites, enemyList — never the HUD scalar's name.",
            "- ISOMETRIC (strategy / RTS / base-builder / tower-defense / tactics — the 'Clash of Clans' angle, NOT flat top-down): if artDirection/perspective says isometric or three-quarter, import { createIsoGrid, isoSort } from './iso.ts' and build on the diamond grid — DON'T hand-roll the projection (that math is where it goes wrong). const iso = createIsoGrid({ tileW: 64, originX: width/2, originY: 80 }); iso.drawGround(ctx, cols, rows, { fillA, fillB, stroke }) for the battlefield floor; iso.place(ctx, sprite, col, row) to stand a unit/building on a cell (feet anchored); iso.toGrid(px, py) + Math.floor to find the cell under a tap; and CRITICAL: render entities via isoSort(units, u => ({ col: u.col, row: u.row })) so nearer units paint over farther ones (un-sorted iso looks broken — back units drawn over front ones). The tilted iso angle is what makes a strategy game look premium; never fall back to flat straight-down for these.",
        ] : []),
