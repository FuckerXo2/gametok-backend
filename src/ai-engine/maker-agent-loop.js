...
export function buildThreeDRulesBlock(foundation = null) {
    ...
    return [
        ...
        "- FREE-ROAM 3D (swim / open flight — dolphin, fish, bird, drone, submarine): the player moves in ALL THREE dimensions — forward along its facing (thrust or gentle auto-swim) PLUS steer UP/DOWN (pitch) and LEFT/RIGHT (turn or strafe). NEVER lock a swimmer/flyer to a single left-right axis. Make the swim/fly volume roomy enough to actually travel through toward the creatures/goals — not a tiny bounding box the player only slides inside.",
        "- HUD: DOM in #hud, not canvas text. Use fixed-width fonts.",
        "- ON-SCREEN CONTROLS (MANDATORY): render visible touch controls in #controls-layer — a thumb JOYSTICK for movement plus a clearly-labelled ACTION/FIRE button (shooters: aim with a right-side drag). NEVER require tapping the player or an entity itself to move/shoot/act. The onboarding hint must point at these controls, and they must be visible on the first frame.",
        ...
        "- THIRD-PERSON / OVER-SHOULDER CAMERA: give the player camera control — horizontal drag rotates the camera yaw around the player (aim) and the chase camera smoothly follows; do NOT hard-lock a single fixed angle. Keep the player framed with the aim direction visible.",
