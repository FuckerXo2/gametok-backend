...
export function buildThreeDRulesBlock(foundation = null) {
    ...
    return [
        ...
        ...((lane.includes('runner') || lane.includes('surfer') || lane.includes('dash')) && isFreeBuildMode() ? [
            '- RUNNER (FREE BUILD — you own the whole game): write the complete src/main.ts for a premium snowboard/runner. Structure it however you think is best — add as many helper functions, entity types, and systems as you want. Aim for a genuinely good-looking, fun game, because the result is what gets shipped. QUALITY BAR: a themed player with a STRONG READABLE SILHOUETTE from the chase camera (body + board + limbs + head, not one box); 2-3 visually DISTINCT obstacle types (e.g. tree, rock, rail) each with its own silhouette and telegraph; at least one collectible/pickup with a clear shape and collect feedback; layered foreground/midground/background scenery so speed and depth read; smooth chase camera with slight follow lag. HARD CONTRACT (keep it runnable + verifiable, or the build is rejected): import * as THREE from "three" and { createThreeStage } from "./threeAssets.ts" and "./styles.css"; get #game-canvas, call createThreeStage(canvas) for renderer/scene/camera (it already provides lights — flat MeshLambertMaterial/MeshBasicMaterial hex colors, no textures, no getDreamTexture); drive an animation loop with requestAnimationFrame that calls renderer.render(scene, camera) every frame; export function stepGame(dt), export function renderAll(), export function resetGame(); and set window.__GAMETOK_TEMPLATE_PROBE__ = { snapshot(){ return { score, gameOver, started, renderCalls: renderer.info.render.calls, ... } }, step(dt){ stepGame(dt); renderAll(); return this.snapshot(); }, reset(){ resetGame(); return this.snapshot(); } }. Touch + keyboard input. One write_file with the full file.',
